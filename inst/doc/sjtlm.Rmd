...
```
`r sjt.lm(fit1, fit2, depvar.labels = c("Barthel-Index", "Negative Impact"), no.output=TRUE)$knitr`
...
sjt.lm(fit1, fit2, show.header = TRUE,
       ...
       depvar.labels = c("Barthel-Index", "Negative Impact"))
```
`r sjt.lm(fit1, fit2, show.header = TRUE, string.est = "Estimate", string.ci = "Conf. Int.", string.p = "p-value", string.dv = "Response", string.pred = "Coefficients", string.interc = "Konstante",depvar.labels = c("Barthel-Index", "Negative Impact"), no.output=TRUE)$knitr`
...
```
`r sjt.lm(fit1, fit2, separate.ci.col = FALSE, show.std = TRUE, p.numeric = FALSE, no.output=TRUE)$knitr`
...
```
`r sjt.lm(fit1, fit2, pred.labels = c("Carer's Age", "Hours of Care", "Carer's Sex", "Educational Status"), no.output=TRUE)$knitr`
...
sjt.lm(fit1, fit2, fit3, 
       ...
       show.fstat = TRUE)
```
`r sjt.lm(fit1, fit2, fit3, separate.ci.col = FALSE, show.aic = TRUE, show.fstat = TRUE, no.output=TRUE)$knitr`
...
```
`r sjt.lm(fit1, fit2, fit3, CSS = list(css.separatorcol = 'padding-right:1.5em; padding-left:1.5em;'), no.output=TRUE)$knitr`
...
```
`r sjt.lm(fit1, fit2, fit3, group.pred = FALSE, no.output=TRUE)$knitr`
...
```
`r sjt.lm(fit1, fit2, fit3, group.pred = FALSE, CSS = list(css.topcontentborder = "+font-size: 0px;"), no.output=TRUE)$knitr`
...
sjt.lm(fit1, fit2, fit3, group.pred = FALSE,
       remove.estimates = 2)
```
`r sjt.lm(fit1, fit2, fit3, group.pred = FALSE, remove.estimates = 2, no.output=TRUE)$knitr`
...
```
`r sjt.lm(fit1, fit2, fit3, group.pred = FALSE, remove.estimates = c("c160age", "c161sex"), no.output=TRUE)$knitr`
...
```
`r sjt.lm(fit1, fit2, fit3, group.pred = FALSE, remove.estimates = c(1,2,5,6,10), no.output=TRUE)$knitr`
...
sjt.lm(fit1, fit2, fit3
       ...
       remove.estimates = c("c160age", "c161sex"))
```
`r sjt.lm(fit1, fit2, fit3, pred.labels = c("mid education", "hi education", "Hours of Care", "slight dependency", "moderate dependency", "severe dependency", "Service Usage"), remove.estimates = c("c160age", "c161sex"), no.output=TRUE)$knitr`
