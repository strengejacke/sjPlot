...
`page.style` and `page.content` are described below in the CSS section. If you want to include a HTML table in your knitr document, simply use the corresponding function within a code chunk. Be sure to use `no.output=TRUE`, because else your output would also be opened in a viewer pane or webbrowser.


If you want to include both the source code and the output in your HTML document, you first need to write your code chunk, which should not be evaluated. Note that you don't need to use `no.output = TRUE` and `$knitr` in this chunk. The results look like this in your HTML document:
...
```{r}
sjt.frq(efc$e42dep, no.output = T)
```
