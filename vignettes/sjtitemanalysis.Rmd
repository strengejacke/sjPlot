...
```
`r sjt.itemanalysis(mydf, no.output=TRUE)$knitr`
...
```
`r sjt.itemanalysis(mydf, factor.groups, no.output=TRUE)$knitr`
...
```
`r sjt.itemanalysis(mydf, factor.groups, show.shapiro = T, show.kurtosis = T, no.output=TRUE)$knitr`
